...
describe('Тестирование страницы связанный список:', () => {
  ...
  it('Проверка корректности добавления элемента в head', () => {
    ...
    cy.get('@addHeadButton').children('img').should('have.attr', 'alt', 'Загрузка.');

    cy.get('@circles').filter(':contains("test")').should('have.css', 'border', '4px solid rgb(210, 82, 225)');
    cy.wait(SHORT_DELAY_IN_MS); // eslint-disable-line
    cy.get('@circles').filter(':contains("test")').should('have.css', 'border', '4px solid rgb(127, 224, 81)');
    cy.wait(SHORT_DELAY_IN_MS); // eslint-disable-line
    cy.get('@circles').filter(':contains("test")').should('have.css', 'border', '4px solid rgb(0, 50, 255)');
  ...
  it('Проверка корректности добавления элемента в tail', () => {
    ...
    cy.get('@addTailButton').children('img').should('have.attr', 'alt', 'Загрузка.');
  ...
  it('Проверка корректности добавление элемента по индексу', () => {
    ...
    cy.get('@addToIndexButton').children('img').should('have.attr', 'alt', 'Загрузка.');
    ...
    cy.wait(SHORT_DELAY_IN_MS); // eslint-disable-line
    cy.get('@circles').eq(2).should('have.css', 'border', '4px solid rgb(127, 224, 81)').contains('test');
    cy.wait(SHORT_DELAY_IN_MS); // eslint-disable-line
    cy.get('@circles').eq(2).should('have.css', 'border', '4px solid rgb(0, 50, 255)').contains('test');
  ...
  it('Проверка корректности удаления элемента из head', () => {
    ...
    cy.get('@deleteHeadButton').children('img').should('have.attr', 'alt', 'Загрузка.');
  ...
  it('Проверка корректности удаления элемента из tail', () => {
    ...
    cy.get('@deleteTailButton').children('img').should('have.attr', 'alt', 'Загрузка.');

    cy.get('@circles').last().should('have.css', 'border', '4px solid rgb(210, 82, 225)');
  ...
  it('Проверка корректности удаления элемента по индексу', () => {
    ...
    cy.get('@deleteToIndexButton').children('img').should('have.attr', 'alt', 'Загрузка.');
    ...
    cy.wait(SHORT_DELAY_IN_MS); // eslint-disable-line
    cy.get('@circles').eq(2).should('have.css', 'border', '4px solid rgb(0, 50, 255)').should('have.text', '');
    cy.get('@circles').eq(3).should('have.css', 'border', '4px solid rgb(210, 82, 225)').should('not.be.empty');
    cy.wait(SHORT_DELAY_IN_MS); // eslint-disable-line
    cy.get('@circles').eq(2).should('have.css', 'border', '4px solid rgb(0, 50, 255)').should('not.be.empty');
