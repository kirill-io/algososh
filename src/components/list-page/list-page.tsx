...
export const ListPage: React.FC = () => {
  const linkedList = useRef(new LinkedList<string>(randomArrayString(3, 6, 0, 100)));
  ...
  const [deleteHeadButtonDisabled, setDeleteHeadButtonDisabled] = useState(true);
  const [deleteTailButtonDisabled, setDeleteTailButtonDisabled] = useState(true);
  const [addByIndexButtonDisabled, setAddByIndexButtonDisabled] = useState(true);
  const [deleteByIndexButtonDisabled, setDeleteByIndexButtonDisabled] = useState(true);
  ...
  useEffect(() => {
    switch (operation) {
      ...
      case Operation.AddByIndex:
        if (inputValue && inputValueRef.current && inputIndexRef.current){
          setCurrentStep(0);        
          setSteps(addByIndex(inputValue, Number(inputIndex), linkedList.current));
          inputValueRef.current.value = ""; 
          inputIndexRef.current.value = "";
          setInputValue(null);
          setInputIndex(null); 
  ...
  const onChangeInputIndexHandler = (e: ChangeEvent<HTMLInputElement>) => {
    if (0 <= Number(e.currentTarget.value) && Number(e.currentTarget.value) < linkedList.current.toArray().length) {
      setInputIndex(e.currentTarget.value);
    } else {
      setInputIndex(null);
    }   

    if (e.currentTarget.value && 0 <= Number(e.currentTarget.value) && Number(e.currentTarget.value) < linkedList.current.toArray().length) {
      ...
      <div className={styles.wrapper}>
        <form className={styles.form}>
          <div className={styles.row}>
            <Input maxLength={4} isLimitText={true} placeholder="Введите значение" extraClass={styles.input} onChange={onChangeInputValueHandler} useRef={inputValueRef} />
            <Button text="Добавить в head" extraClass={styles.value_button} onClick={() => {
                setOperation(Operation.AddHead);
                setIsLoaderButton(Operation.AddHead);
              }} disabled={addHeadButtonDisabled || buttonDisabled} isLoader={isLoaderButton === Operation.AddHead ? true : false} />
            <Button text="Добавить в tail" extraClass={styles.value_button} onClick={() => {
                setOperation(Operation.AddTail);
                setIsLoaderButton(Operation.AddTail);
              }} disabled={addTailButtonDisabled || buttonDisabled} isLoader={isLoaderButton === Operation.AddTail ? true : false} />
            <Button text="Удалить из head" extraClass={styles.value_button} onClick={() => {
                setOperation(Operation.DeleteHead);
                setIsLoaderButton(Operation.DeleteHead);              
              }} disabled={deleteHeadButtonDisabled || buttonDisabled} isLoader={isLoaderButton === Operation.DeleteHead ? true : false} />
            <Button text="Удалить из tail" extraClass={styles.value_button} onClick={() => {
          ...
          <div className={styles.row}>
            ...
            <Button text="Добавить по индексу" extraClass={styles.index_button} onClick={() => {
              ...
              }} disabled={addByIndexButtonDisabled || buttonDisabled} isLoader={isLoaderButton === Operation.AddByIndex ? true : false} />
            <Button text="Удалить по индексу" extraClass={styles.index_button} onClick={() => {
                setOperation(Operation.DeleteByIndex);
                setIsLoaderButton(Operation.DeleteByIndex);
              }} disabled={deleteByIndexButtonDisabled || buttonDisabled} isLoader={isLoaderButton === Operation.DeleteByIndex ? true : false} />
          </div>
        </form>
        <div className={styles.result}>
          {steps &&
            steps[currentStep].list.map((item: ILinkedListNode<string>, i: number, array: ILinkedListNode<string>[]) => {
              return(
                <div className={styles.item} key={i}>
                  <Circle 
                    letter={steps[currentStep].hideValue[i] ? undefined : item.getValue}
                    head={i === steps[currentStep].index && steps[currentStep].direction === Position.Top ? <Circle letter={steps[currentStep].value} isSmall={true} state={ElementStates.Changing} /> : i === 0 ? 'head' : null} 
                    tail={i === steps[currentStep].index && steps[currentStep].direction === Position.Bottom ? <Circle letter={steps[currentStep].value} isSmall={true} state={ElementStates.Changing} /> : i === steps[currentStep].list.length - 1 ? "tail" : null}
                    index={i} 
                    key={i}
